...
struct PitchRhythm{
	vector<int> progress;  // 进度快慢，为1表示快了，0表示正确，-1表示慢了
	vector<double> during; // 音符持续时间
	vector<int> pitch;    //音符
	int location;        // 定位
};
...
public:
	...
	* @author lijun
	* @date  2017/12/25
	* @return 返回数组记录之前弹奏过正确音符的节奏评价
	* @brief 每个音符的节奏快慢评价
	*/
	vector<PitchRhythm> EvaluatePerNoteRhythm();
    /**
     * @author zhangqianyi
     * @date  2017/6/16
     * @return 返回评价之后的分数
	...
	double CountScore(vector<Correctness> correctness, vector<PitchRhythm>	NoteThythm);
	...
	void SaveDuratinTime(const string &filePath);

	void SetSpectrum(vector<vector<double>> SpectrumOffLine);

	void ReadpitchFre(const string &pitchFrePath);
private:
    /**
     * @author zhangqianyi
     * @date  2017/6/5
	...
	int MaxNumPitchDuration;           //整首曲子里面最多的音符时长类型 1/2 ，1/4等其中的一种

	vector<vector<double>> timeduration;

	double AveMaxNumPitchDuration;          //音符时长最多类型时长的平均值

	vector<vector<double>> Spectrum;        // 整首曲子的频谱

	vector<double> pitchFre;               //每个钢琴键的频率
