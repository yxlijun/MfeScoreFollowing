...
public:
	...
	*/
	void ScoreFollowingOffline(const vector<vector<double> > &H, vector<double> & error, vector<vector<double>> Ratio, double timeResolution, int maxPitchesInEvent);
	...
	*/
	...
	bool isDeletePitch(vector<double> &totalratio, double nowratio);   //返回是否滤除人声操作
