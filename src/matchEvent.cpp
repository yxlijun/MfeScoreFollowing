	...
	if (iEvent == -1){
		if ((pitch.size() == scoreEventPitches.size()) && (pitch.size() == 1)){
			...
			if (pitchoctive == scorePitchOctive){
				iEvent = iEventPre;
				vector<double>::iterator it = find(scoreEventPitches.begin(), scoreEventPitches.end(), pitch[0]);
				if (it != scoreEventPitches.end()) {
					lia[0] = 1; // 表示当前音符存在
					locb[0] = it - scoreEventPitches.begin(); // 给出是第几个
				}
			}
		}
		...
		if (iEvent == -1 && pitch.size()>scoreEventPitches.size()){
			...
			for (int i = 0; i < pitch.size(); i++){
				bool findSame = find(scoreEventPitches.begin(), scoreEventPitches.end(), pitch[i]) == scoreEventPitches.end();
				if (findSame)
					omssionOctive.push_back(pitch[i] % 12);
			}
...
int isIEventAll(vector<int> pitch, const vector<vector<vector<double>>>& scoreEvent, int iEventPre, vector<int>& lia, vector<int>& locb){
	int iEvent;
	// 乐谱当前位置演奏的音符
	vector<double> scoreEventPitches(scoreEvent[iEventPre][g_Pitches].begin(), scoreEvent[iEventPre][g_Pitches].end());
	if (pitch.size() != scoreEventPitches.size()){
