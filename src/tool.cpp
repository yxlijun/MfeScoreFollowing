...
void Genexcel(string diroot){
	Py_Initialize();	//使用python之前，要调用Py_Initialize();这个函数进行初始化
	PyObject * pFunc = NULL;	// 声明变量
	PyObject * pModule = PyImport_ImportModule("drawf");	//这里是要调用的文件名
	pFunc = PyObject_GetAttrString(pModule, "excel");	//这里是要调用的函数名
	PyObject* args = PyTuple_New(1);
	PyObject* arg1 = Py_BuildValue("s", diroot.c_str()); // 字符串参数
	PyTuple_SetItem(args, 0, arg1);
	PyEval_CallObject(pFunc, args);//调用函数
	Py_Finalize();//调用Py_Finalize，这个根Py_Initialize相对应的
}