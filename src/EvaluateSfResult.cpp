...
void EvaluateSfResult::Init()
{
	...
	// 获取每个音符弹奏的时长
	//vector<vector<double>>().swap(timeduration);
	//map<int, vector<double>> timepitchNum;
	//for (int i = 0; i < timePitchesPair.size(); i++){
	//	if (timePitchesPair[i].size() > 0){
	//		vector<double> temp;
	//		for (int j = 0; j < timePitchesPair[i].size(); j++){
	//			if (timePitchesPair[i][j].size() == 4){
	//				temp.push_back(0);
	//				temp.push_back(timePitchesPair[i][j][2] - timePitchesPair[i][j][0]);
	//			}
	//		}
	//		if (!temp.empty()) timepitchNum[i + 1] = temp;
	//	}
	//}
	//for (int i = 0; i < sfResult[0].size(); i++){
	//	vector<double> nowtimeDuration;
	//	for (int j = 1; j < sfResult[0][i].size(); j += 2){
	//		int index = static_cast<int>(sfResult[0][i][j]);
	//		for (int z = 0; z < timepitchNum[index].size(); z += 2){
	//			if (timepitchNum[index][z] == 0){
	//				nowtimeDuration.push_back(index);
	//				nowtimeDuration.push_back(timepitchNum[index][z + 1]);
	//				timepitchNum[index][z] = 1;
	//				break;
	//			}
	//		}
	//	}
	//	timeduration.push_back(nowtimeDuration);
	//}
	//map<int, int> PitchDurationNum;   //每种时长对应的次数
	//for (int i = 0; i < scoreEvent.size(); i++){
	//	for (int j = 0; j<scoreEvent[i][g_Duration].size(); j++){
	//		PitchDurationNum[static_cast<int>(scoreEvent[i][g_Duration][j])]++;
	//	}
	//}
	//// 获取时长出现最多的类型
	//int tempNum = 0;
	//for (map<int, int>::iterator it = PitchDurationNum.begin(); it != PitchDurationNum.end(); it++){
	//	if (it->second > tempNum){
	//		MaxNumPitchDuration = it->first;
	//		tempNum = it->second;
	//	}
	//}
}
...
vector<Correctness> EvaluateSfResult::EvaluateCorrectness()
{
    ...
    for (vector<int>::size_type i = 0; i < sfResult[2].size(); ++i) {
		...
		for (int z = 1; z <= 2; z++){
			if (location - z - 1 >= 0){
				for (vector<int>::size_type j = 0; j <scoreEvent[location - z - 1][g_Pitches].size(); j++) {
		...
		for (int z = 0; z <2; z++){
			if (location + z < scoreEvent.size()){
				for (vector<int>::size_type j = 0; j < scoreEvent[location + z][g_Pitches].size(); j++) {
...
vector<Correctness> EvaluateSfResult::EvaluateCorrectnessModify()
{
	...
	for (vector<int>::size_type i = 0; i < sfResult[2].size(); ++i) {
		...
		vector<int> nextOctavesInScore;  // 下一个位置的octave
		vector<int> currOctavesInScore; // 当前位置的octave
		set<int> curPitchUpDown;    //当前位置乐谱音符中上下一个位置的音
		set<int> lastPitchUpDown; //上一个位置乐谱音符中上下一个位置的音
		vector<int> nearPitch;    //当前演奏音符中与多弹音符距离为1的音符
		if (!correctness->excess.empty()){
			for (set<int>::iterator pos = pitchesInScore.begin(); pos != pitchesInScore.end(); ++pos){
				curPitchUpDown.insert(*pos - 1);
				curPitchUpDown.insert(*pos + 1);
			}
			for (vector<int>::iterator it = correctness->excess.begin(); it != correctness->excess.end(); ++it){
				for (set<int>::iterator pos = pitchesPerformance.begin(); pos != pitchesPerformance.end(); ++pos){
					if (*it != *pos){
						curPitchUpDown.insert(*pos - 1);
						curPitchUpDown.insert(*pos + 1);
					}
					if (abs(*it - *pos) == 1) nearPitch.push_back(*pos);
				}
				for (vector<int>::size_type j = 1; i >= 1 && j < sfResult[0][i - 1].size(); j += 2) {
					if (*it != static_cast<int>(sfResult[0][i - 1][j])){
						lastPitchUpDown.insert(static_cast<int>(sfResult[0][i - 1][j] - 1));
						lastPitchUpDown.insert(static_cast<int>(sfResult[0][i - 1][j] + 1));
					}
				}
				for (vector<int>::size_type j = 1; j < sfResult[0][i].size(); j+=2){
					if (*it != static_cast<int>(sfResult[0][i][j])){
						curPitchUpDown.insert(static_cast<int>(sfResult[0][i][j] - 1));
						curPitchUpDown.insert(static_cast<int>(sfResult[0][i][j] + 1));
					}
				}
			}
		...
		for (vector<int>::iterator it = correctness->excess.begin(); it != correctness->excess.end();) {
			...
			int dis = 0;  //用来记录当前多弹音符在上一次弹该音符之间的距离
			for (vector<Correctness>::iterator it2 = totalCorrectness1.begin(); it2!=totalCorrectness1.end(); it2++){
				index1++;
				for (vector<int>::size_type j = 0; j < it2->excess.size(); j++){
					if (it2->excess[j] == *it)
			...
			bool durationMore = false;
			if (timePitchesPair[*it - 1][counts].size()==4)
				durationMore = (timePitchesPair[*it - 1][counts][2] - timePitchesPair[*it - 1][counts][0])<=0.1;
			bool MinusKeepPitch = ((NowHpeakerMax * 2) < PrevHpeakerMax && dis<=7);
			//cout << *it << "..." << PrevHpeakerMax << "..." << NowHpeakerMax << "...."<<MinusKeepPitch<<endl;
			// 是否是上一个位置的倍频错误
			//bool findcurUpdown = find(curPitchUpDown.begin(), curPitchUpDown.end(), *it) != curPitchUpDown.end();
			//bool findlastUpdown = find(lastPitchUpDown.begin(), lastPitchUpDown.end(), *it) != lastPitchUpDown.end();
			bool findTwoFre = false;
			if (!nearPitch.empty()){
				for (vector<int>::iterator nit = nearPitch.begin(); nit != nearPitch.end(); ++nit){
					int dis = std::distance(sfResult[0][i].begin(), find(sfResult[0][i].begin(), sfResult[0][i].end(), *nit));
					double onset = sfResult[0][i][dis-1];
					int onsetframe = onset * 44100 / static_cast<double>(512);
					double excessFre = pitchFre[*it - 1] * 2;
					double nearFre = pitchFre[*nit - 1] * 2;
					int excessInSpectrum = excessFre * 4096 / static_cast<double>(44100)+1;
					int nearInSpectrum = nearFre * 4096 / static_cast<double>(44100)+1;
					int endframe = onsetframe + 3 < Spectrum.size() ?onsetframe + 3 : Spectrum.size() - 1;
					vector<double> onsetSpectrum(513);
					for (int k = onsetframe; k <=endframe; k++){
						vector<double> temp;
						transform(Spectrum[k].begin(), Spectrum[k].end(), onsetSpectrum.begin(), back_inserter(temp), plus<double>());
						onsetSpectrum = temp;
					}
					for (int k = 0; k < onsetSpectrum.size(); k++){
						onsetSpectrum[k] /= static_cast<double>(4);
					}
					if (excessInSpectrum < onsetSpectrum.size() && nearInSpectrum < onsetSpectrum.size())
						findTwoFre = onsetSpectrum[excessInSpectrum] < 0.3*onsetSpectrum[nearInSpectrum];
				}
			}
			if ((findInLast || findInLastOctave || findInOctave || findInNext || findInNextOctave || isSureFlag || isMutifreq || MinusKeepPitch || durationMore || findTwoFre)) {
				it = correctness->excess.erase(it);
			}
			else ++it;
		...
		for (vector<int>::iterator it = correctness1.excess.begin(); it != correctness1.excess.end();) {
			...
			// 是否是上一个位置的倍频错误
			if ((findInLast || findInLastOctave || findInOctave || findInNext || findInNextOctave || isSureFlag || isMutifreq || findTwoFre)) {
				it = correctness1.excess.erase(it);
			}
			else ++it;
...
vector<BeatRhythm> EvaluateSfResult::EvaluateBeatRhythm()
{
	...
	/*for (int i = 0; i < scoreEvent.size(); i++){
		if (scoreEvent[i][g_Tied][0] == 1){
			int tienum = 0;
			while (tienum< barFirst.size() && i<sfResultLocate.size() && !(sfResultLocate[i] >= barFirst[tienum] && sfResultLocate[i] <= barEnd[tienum])){
				++tienum;
			}
			WrongBeat.push_back(tienum);
		}
	}*/
...
vector<PitchRhythm> EvaluateSfResult::EvaluatePerNoteRhythm(){
	vector<PitchRhythm>	NoteThythm(sfResult[2].size());
	int SumEqualMaxduration = 0;
	...
	for (vector<int>::size_type i = 0; i < sfResult[2].size(); ++i) {
		...
		int *end = set_intersection(pitchesInScore.begin(), pitchesInScore.end(), pitchesPerformance.begin(),
			pitchesPerformance.end(), intersection);
		if (static_cast<int>(sfResult[1][i][0]) == 1 && sfResult[0][i].size()==timeduration[i].size()){
			for (int *p = intersection; p != end; ++p) {
				for (int j = 0; location <= scoreEvent.size() && j < scoreEvent[location - 1][g_Pitches].size(); ++j) {
					if (*p == static_cast<int>(scoreEvent[location - 1][g_Pitches][j])){
						if (scoreEvent[location - 1][g_Duration][j] == MaxNumPitchDuration){
							for (int m = 1; m < sfResult[0][i].size(); m += 2){
								if (sfResult[0][i][m] == *p){
									Sumduration += timeduration[i][m];
									SumEqualMaxduration++;
								}
							}
						}
	...
	SumEqualMaxduration = SumEqualMaxduration > 0 ? SumEqualMaxduration : 1;
	AveMaxNumPitchDuration = Sumduration / (static_cast<double>(SumEqualMaxduration));
	double noteleranceRate = AveMaxNumPitchDuration*0.5;
	for (int i = 0; i < sfResult[0].size(); i++){
		int location = static_cast<int>(sfResultLocate[i]); // 当前演奏的音符在乐谱中的位置
		// 遍历当前位置演奏的音符
		set<int> pitchesPerformance;
		...
		vector<int> interPitch;
		int intersection[20]; // 交集，预分配20大小，演奏的音符和乐谱中相应位置的音符交集不会超过20个音符
		int *end = set_intersection(pitchesInScore.begin(), pitchesInScore.end(), pitchesPerformance.begin(),
			pitchesPerformance.end(), intersection);
		for (int *p = intersection; p != end; ++p){
			interPitch.push_back(*p);
		}
		if (static_cast<int>(sfResult[1][i][0]) == 1 && sfResult[0][i].size() == timeduration[i].size()){
			for (int index = 0; index < interPitch.size(); index++){
				for (int j = 0; location <= scoreEvent.size() && j < scoreEvent[location - 1][g_Pitches].size(); ++j) {
					if (interPitch[index] == static_cast<int>(scoreEvent[location - 1][g_Pitches][j])){
						for (int m = 1; m < sfResult[0][i].size(); m += 2){
							if (interPitch[index] == sfResult[0][i][m]){
								double durationratio = static_cast<double>(static_cast<double>(scoreEvent[location - 1][g_Duration][j]) / static_cast<double>(MaxNumPitchDuration));
								if (fabs(durationratio*AveMaxNumPitchDuration - timeduration[i][m])<noteleranceRate)  NoteThythm[i].progress.push_back(0);  //节奏正常
								else if (timeduration[i][m] - durationratio*AveMaxNumPitchDuration<noteleranceRate)  NoteThythm[i].progress.push_back(1);   //节奏快了 
								else if (timeduration[i][m] - durationratio*AveMaxNumPitchDuration>noteleranceRate)  NoteThythm[i].progress.push_back(-1);  //节奏慢了
								NoteThythm[i].pitch.push_back(interPitch[index]);
								NoteThythm[i].during.push_back(timeduration[i][m]);
							}
						}
					}
				}
		...
		if (NoteThythm[i].progress.empty()){
			for (vector<int>::size_type j = 0; j < timeduration[i].size();j+=2) {
				NoteThythm[i].pitch.push_back(static_cast<int>(timeduration[i][j]));
				NoteThythm[i].during.push_back(timeduration[i][j+1]);
				NoteThythm[i].progress.push_back(0);
			}
		}
		NoteThythm[i].location = static_cast<int>(sfResult[2][i][0]);
	}
	return NoteThythm;
}


double EvaluateSfResult::CountScore(vector<Correctness> correctness, vector<PitchRhythm> NoteThythm)
{
	//if (correctness.empty() || NoteThythm.empty()) return 0;
	...
	//double scorePerBeat = beatRhythmScore / static_cast<double>(NoteThythm.size()); // 前面进行了为空判断，可以直接除
	//for (vector<int>::size_type i = 0; i < NoteThythm.size(); ++i) {
	//	for (vector<int>::size_type j = 0; j < NoteThythm[i].progress.size(); ++j){
	//		if (NoteThythm[i].progress[j] == 1 || NoteThythm[i].progress[j] == -1){
	...
	if (correctness.size() > scoreEvent.size() / 2){
		if (scorecorrect == 0){
			if (beatcorrect >= 0 && beatcorrect <= 10)
				finalScore = 5;
			else if (beatcorrect > 10 && beatcorrect <= 30)
				finalScore = 4;
			else
				finalScore = 3;
		}
...
void EvaluateSfResult::SetSpectrum(vector<vector<double>> SpectrumOffLine){
	Spectrum = SpectrumOffLine;
}

void EvaluateSfResult::ReadpitchFre(const string &pitchFrePath){
	fstream fin(pitchFrePath);
	if (!fin.is_open()){
		cout << "cann't open pitchFrefile" << endl;
		return;
	}
	while (fin){
		double data;
		fin >> data;
		pitchFre.push_back(data);
	}
	pitchFre.erase(pitchFre.end() - 1);
	fin.close();
}

void EvaluateSfResult::SaveDuratinTime(const string &filePath){
	ofstream fileStream(filePath);
	if (!fileStream.is_open()){
		cout << "Error opening file PitchDurationResult" << endl;
		return ;
	}
	fileStream << "检测音符\t" << "onset\t" << "音符时长" << endl;
	for (vector<int>::size_type i = 0; i < sfResult[0].size(); i++){
		for (vector<int>::size_type j = 1; j < sfResult[0][i].size(); j+=2){
			fileStream << static_cast<int>(sfResult[0][i][j]) << " ";
		}
		fileStream << "\t"; 
		...
		fileStream << "\t";
		for (vector<int>::size_type j = 1; j <timeduration[i].size() ; j+=2){
			fileStream << timeduration[i][j] << " ";
		}
		fileStream << endl;
	}
	fileStream.close();
...
void EvaluateSfResult::SaveCorrectness(const string& filePath,vector<Correctness> correctness){
	Json::Value root;
	for (int i = 0; i < correctness.size(); i++){
		Json::Value data;
...
void EvaluateSfResult::SaveBeatRhythm(const string& filePath, vector<BeatRhythm> beatRhythm){
	Json::Value root;
	for (vector<int>::size_type i = 0; i < beatRhythm.size(); i++){
		Json::Value data;
...
void EvaluateSfResult::SavePlayBack(const string& filePath){
	Json::Value root;
