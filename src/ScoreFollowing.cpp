...
void ScoreFollowing::ScoreFollowingOffline(const vector<vector<double> > &H, vector<double> & error, vector<vector<double>> Ratio,double timeResolution, int maxPitchesInEvent)
{
	...
	for (vector<int>::size_type i = 0; i < H.size(); ++i) {           
		...
		if (flag) {
			if (Ratio[i][0]>=0.1 || Ratio[i][1]>=50 /*|| Ratio[i][2]>60*/){
...
bool ScoreFollowing::isDeletePitch(vector<double> &totalratio,double nowratio){
	int totalsize = totalratio.size();
	int count = 0;
	if (totalsize>10){
		for (vector<int>::size_type i =totalsize-1 ; i >totalsize-11; i--){
			if (totalratio[i] >= 60){
				count++;
			}
		}
...
void ScoreFollowing::MinusDoubleFreq(vector<int> &pitches, int location){
	...
	for (vector<int>::size_type i = 0; i < newpitches.size(); i++){
		bool findPitch = find(scorepPitch.begin(), scorepPitch.end(), newpitches[i]) == scorepPitch.end();
		bool findOctive = find(scoreOctive.begin(), scoreOctive.end(), newpitches[i] % 12) != scoreOctive.end();
		if (findPitch && findOctive){
			vector<int>::iterator pos = remove(newpitches.begin(), newpitches.end(), newpitches[i]);
			newpitches.erase(pos, newpitches.end());
		}
	}
	vector<int> ObservaPitches;
	vector<int> ObservaOctive;
	int begin = location - 2 >= 0 ? location - 2 : 0;
	int end = location + 2 < scoreEvent.size() ? location + 2 : scoreEvent.size() - 1;
	for (int i = begin; i <=end; i++){
		for (int j = 0; j < scoreEvent[i][g_Pitches].size(); j++){
			ObservaPitches.push_back(static_cast<int>(scoreEvent[i][g_Pitches][j]));
			ObservaOctive.push_back(static_cast<int>(scoreEvent[i][4][j]));
		}
	}
	for (vector<int>::size_type i = 0; i < newpitches.size(); i++){
		bool findPitch = find(ObservaPitches.begin(), ObservaPitches.end(), newpitches[i]) == ObservaPitches.end();
		bool findOctive = find(ObservaOctive.begin(), ObservaOctive.end(), newpitches[i] % 12) != ObservaOctive.end();
		bool PitchLess20 = (newpitches[i] < 20);
		if ((findPitch && findOctive) || (PitchLess20 && findPitch)){
			vector<int>::iterator pos = remove(newpitches.begin(), newpitches.end(), newpitches[i]);
			newpitches.erase(pos, newpitches.end());
		}
	}
...
void ScoreFollowing::findReplay(){
	...
	for (int i = 0; i < loctionOffline.size(); i += 2){
		...
		double maxMatch = *max_element(matchSum.begin(), matchSum.end());
		if (matchSum.empty()){
			return;
		}
		int maxLoc = 0;
		...
		for (int j = loctionOffline[i]; j <= loctionOffline[i + 1]; j++){
			sfResult[2][j][0] = maxLoc + j - loctionOffline[i];
		}
	}
	/*for (int i = 0; i < sfResult[2].size(); i++){
		map<int, int>::iterator iter;
		for (iter = scoreLocate.begin(); iter != scoreLocate.end(); iter++){
			if (iter->second == sfResult[2][i][0]){
				sfResultLocate[i] = iter->first;
				break;
			}
		}
	}	*/
