...
vector<vector<int> > scoreFollowingEvent(vector<int>& pitch, vector<vector<vector<double> > >& scoreEvent, vector<int> &sfResultOriginLocate,
	...
	while (isSureFlagPre == 1) {
		...
		if (isCurr != -1 && !anyIsPlayed) { // 和当前位置匹配，且都未被演奏过，确定为当前位置
			if (iEventPre > 0 && iEventPre < scoreEventSize - 1){
				int isNext = isIEventTotal(pitch, scoreEvent, iEventPre + 1, lia2, locb2);
				int isNext1 = isIEvent(pitch, scoreEvent, iEventPre + 1, lia2, locb2);
				if (isNext != -1 || isNext1!=-1){
					isCurr = iEventPre + 1;
			...
			if (iEventPre < scoreEventSize - 1) {
				...
				int isNext = -1;
				if (iEventPre >= 0){
					isNext1 = isIEvent(pitch, scoreEvent, iEventPre + 1, lia2, locb2);
					isNext = isIEventTotal(pitch, scoreEvent, iEventPre + 1, lia2, locb2);
				}
				if (isNext != -1 || isNext1 != -1){
					isCurr = iEventPre + 1;
					vector<vector<int> >().swap(iEvent);
		...
		if (iEventPre < scoreEventSize - 1) {
			if (sfResultOriginLocate.size() > 0){
				...
				bool is_sort = false;
				if (sfResultOriginLocate.size() >= 2)
					is_sort = is_sorted(sfResultOriginLocate.end() - 2, sfResultOriginLocate.end());
				if (repeatnum >= 2 && is_sort){
			...
			if (iEventPre + 2 < scoreEvent.size()){
				isNext2 = isIEventAll(pitch, scoreEvent, iEventPre + 2, lia2, locb2);
			}
				...
				if (repeatnum >= 2 && is_sort){
					for (vector<int>::size_type i = 1; i <= repeatnum; i++){
						...
						if (ieventnow < scoreEventSize){
							...
							if (isNext != -1){
								vector<vector<int> >().swap(iEvent);
								iEvent.push_back(vector<int>{ieventnow}); // 将iEvent换成下一个位置
								lia = lia2;
								locb = locb2;
							}
						}
	...
	if (isSureFlag == 0) {	
		...
		int iEventLastSure1 = iEventLastSure >= 1 ? iEventLastSure - 1 : 0;
		int barFirst = static_cast<int>(scoreEvent[iEventLastSure1][g_BarFirst][0]) - 1;
		//        if (notSureFlag < 0) {
